...
# 热门榜
def rank_by_time(mode='day_male', num=1):    # (mode, date = None, offset = None) 作品排行
...
def main(text):
    ...
    try:
        ...
        # 寻找当天r18色图
        donot_rush = ["别冲了", "营养跟得上吗?", "你撸多了", "别撸了,伤身体", "再冲会暴毙信不", "求求你别冲了", "这么多你营养跟得上?", "求求你别冲了", "冲这么多你今晚biss", "天天看色图不去读书?", "再冲jb都给你剁了", "朋友少撸管多学习"]
        img_lst = []
        ...
        # 按tag搜寻:
            ...
            # 列出推荐tag列表
                ...
                output['img_list'], output['text'] = search_by_tag_popular(word=tags_list[randint(0, 39)])
            # 直接搜寻tag
            elif text.startswith('tag '):
                word = text[4:]
